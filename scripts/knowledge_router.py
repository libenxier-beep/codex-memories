...
MAX_GIT_BATCH_INPUT_BYTES = 1024
MAX_GIT_BATCH_OUTPUT_BYTES = 4 * 1024 * 1024
    ...
    with tempfile.TemporaryDirectory(prefix="knowledge-route-") as temporary:
        ...
        while offset < len(entries):
            ...
            while end < len(entries):
                ...
                if end > offset and (
                    predicted_input_bytes + predicted_input_entry_bytes > MAX_GIT_BATCH_INPUT_BYTES
                    or predicted_bytes + predicted_entry_bytes > MAX_GIT_BATCH_OUTPUT_BYTES
                ):
                    break
...
def _virtual_collection_registry(registry: dict[str, Any]) -> dict[str, Any]:
        ...
        contexts.append(
            {
                ...
                "routing_profile": collection["routing_profile"],
                "read_path": [path],
                "deeper_files": [],
                "deeper_routes": [],
...
def route_knowledge(
    ...
    *,
    ...
    selected = engine.route(query, _virtual_collection_registry(registry), root=None)
    if selected["decision"] != "load":
        ...
        result = {
            ...
            # Bind the public receipt to the exact caller query.  The nested
            # router may normalize punctuation/case for ranking, so its own
            # fingerprint is not an authority identity for the raw request.
            "query_fingerprint": _query_fingerprint(query),
