...
def _graph_candidates(
    ...
    *,
    ...
    if graph_id is None or not isinstance(context_id, str):
        return {
...
def access_knowledge(
    ...
    *,
    ...
    if mode == "durable":
        try:
            privacy_route = route_knowledge(
                query, root=trusted_router_root, read_selector=None
            )
            request = verify_recall_request(
                ...
                + hashlib.sha256(
                    (
                        ...
                        + "\0"
                        + str(trusted_router_root.resolve(strict=False))
                    ).encode("utf-8")
                ).hexdigest(),
            )
        except Exception:
        ...
        hybrid = GovernedHybridRetrieval(
            ...
            embedding=LocalNaturalLanguageEmbedding(
                helper_source=AGENT_MEMORY_EMBEDDING_HELPER,
                cache_dir=AGENT_MEMORY_EMBEDDING_CACHE,
            ),
        )
        try:
    ...
    route = route_knowledge(
        query, root=trusted_router_root, read_selector=read_selector
    )
    privacy_stop = route.get("trace", {}).get("stage") == "privacy_boundary"
    backend = route.get("trace", {}).get("retrieval_backend", "not_run")
    graph = (
        _graph_candidates(route, graph_root=graph_root, limit=limit)
        if expand_graph
        else {
            "status": "skipped",
    ...
    semantic = {
        ...
        "backend": None,
        "reason": "privacy_boundary" if privacy_stop else "no_configured_embedding_index",
        "candidate_only": True,
    }
    return {
        "schema_version": 1,
